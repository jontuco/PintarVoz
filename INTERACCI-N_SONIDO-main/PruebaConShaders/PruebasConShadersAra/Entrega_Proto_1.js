...
//-------VARIABLES QUE ALMACENAN GLSL----
let vert = `
precision highp float; //esto no es más que la precision del calcula, maxima cantidad de decimales despues de la coma

attribute vec3 aPosition; //variable de tipo attribute, exclusiva del vertex, significa que recibe informacion por CADA vertice de la geometria, en este caso las tres coordenadas de posicion
attribute vec2 aTexCoord; //recibe las dos coordenadas uv de cada vertice, esto lo va a necesitar el fragment para colorear. son valores normalizados e 0 y 1

varying vec2 vTexCoord; //variable tanto de vertex como de fragment, sirve para compartir informacion desde el vertex al fragment, aca vamos a almacenar las coordenadas uv recibidas desde la cpu, y la gpu las va a interpolar/compartir con el fragment al declarar la variable homonima

uniform mat4 uModelViewMatrix; //variable de valor constante, crea una matriz de 4*4 (16 valores), sirve para hacer transformaciones 3d, posicion, rotacion, escala y camara
uniform mat4 uProjectionMatrix; //proyecta la imagen despues de ser acomodada por uViewMatrix
uniform float time; //almacena el tiempo en millis, el valor de la variable lo definimos en el draw usando p5js no glsl, en este caso corrsponde a ondasShader.setUniform("time", millis());

void main() {  //esto es lo que se ejecuta del codigo glsl, lo anterior son solo valores de iniciacion, no se, yo lo pienso como si lo anterior fueran variables globales y preloads y esto fuera el setup o mejor el draw, pero la gracia de la gpu es que no va procesando linea por linea sino que todo el codigo se compila y se procesa para cada vertice y fragmento al mismo tiempo porqwue la gpu funciona con procesamiento paralelo porque tiene muchisimos mas nucleos que una cpu
  vTexCoord = aTexCoord; //aca le estamos diciendo que tome cada paquete de dos valores de las coordenadas uv y las almacene como varing para interpolarlas

  vec4 viewModelPosition = uModelViewMatrix * vec4(aPosition, 1.0); //aca hasta donde entiendo lo que hacemos es convertir nuestro vec3 en un vec4 que almacena un valor de la dimension w que en este caso va a ser siempre 1.0 (ojo que glsl es un lenguaje de tipado fuerte osea que si no ponemos punto y cero el compilador va a creer que es un entero y va a dar error porque se espera un flotante, cuidadito), esto se necesitaria para poder operar la matriz 4*4 por un vec4, you know 4 valores por 4 valores o algo asi. transorma la posicion en de tres coordenadas en una posicion de la camara de cuatro dimensiones
  viewModelPosition.x += 1.0 * sin(time * 0.01 + viewModelPosition.y * 0.1); //aca estamos redefiniendo la coordenada x de cada vertice, su posicion depende del seno del tiempo de la posicion en el eje y y de un valor constante

  gl_Position = uProjectionMatrix * viewModelPosition; //esta es una variable especial, hoy estamos usando esta sintaxis pero por lo que pude leer esta obsoleta, ahora parece que lo que se hace es declarar antes del void main una variable out, que tiene la capacidad de salir como valor nuevo desde el shader hacia la cpu, esa seria la buena practica actual en glsl
}
`;

let frag = `
precision highp float; //basicamente lo mismo que ebn el vertex
...
//    
//esta variable va a ser el nombre del archivo JSON que podemos pensarlo como una bolsa llena de cosas, si no recordamos el nombre de la cosa no podemos sacarlo
//contiene colores y podria contener coordenadas 
//los colores estan en arreglos bidimensionales, podes elegir con que valor trabajar del color HSB

//esta variable es el nombre del la estructura que vamos a sacar del JSON, en este caso un arreglo bidimensional, todito, 
// un indice para colores completos, 123 en total
// un subindice para elegir entre sus tres valores que son  H S B 
// H es el rango del espectro visible
// S es la saturacion    ////esta variable junto a la opacidad del color nos ayudan a dibujar las transiciones entre las obras de la serie
// B es el brillo/luminosidad
          

//si quiero usar un color del arreglo primero hay que declararlo []
//paletaE1 = datosHSB.coloresE1
//  //ahora paletaE1 es el arreglo de colores
//para aplicar el color tengo que...cargar el color..recomponerlo...H...S...B
//y hay dos dimensiones del arreglo (123 y 3)
//puede ser con random
//puede ser con for
//o clasico...paletaE1[74][3];



/*
for (let i = 0; i < datosHSB.paletaE1.length; i++) {
  let h = datosHSB.paletaE1[i][0];
  let s = datosHSB.paletaE1[i][1];
  let b = datosHSB.paletaE1[i][2];
  //aca agarrate, traje un target porque no se trabajar solo con uno
  if (int)
    //                          pueden cambiarse por variables tipo "calibracion"
    //                                     \/    \/
    let targetTransicion = map(intensidad, 0.2, 0.8, 80, 10, true); //

  if (int)

    colorHSBe1[i] = color(h, s, b);
}

for (let i = 0; i < datosHSB.puntos.length; i++) {
  beginShape();
  curveVertex(this.puntos[i] + random(-vib, vib), this.puntos[i + 1] + random(-vib, vib))
  endShape(CLOSE);
}
*/
...
//-------SHADER----
let ondasShader; //aca almacenamos el shader
let apagar = false;

function preload() {
  //ondasShader = loadShader(); //ESTO NO NOS INTERESA PORQUE SERIA PARA CARGAR SHADERS PRE DEFINIDOS Y NOSOTRES QUEREMOS HACER NUESTROS PROPIOS SHADERS
}

function setup() {
  createCanvas(800, 600, WEBGL);
  secBuffer = createGraphics(800, 600, WEBGL);
  ondasShader = createShader(vert, frag);
  shader(ondasShader); //aca declaramos el uso del shader, se va a aplicar a todo lo que se dibuje despues
  noStroke();
  mouseXant = 0;
  mouseYant = 0;
...
function draw() {
  ...
  ondasShader.setUniform('time', millis());
...
function keyPressed() {
  //para activar y desactivar el shader
  if (key === 's' || key === 'S') {
    apagar = !apagar;
  }
...
class Curva {
  constructor(id, col, puntos) {
    ...
    // centroide precomputado: puntos nunca cambia, no tiene sentido recalcular cada frame
    let cx0 = 0, cy0 = 0;
    for (let i = 0; i < puntos.length; i += 2) {
      cx0 += puntos[i];
      cy0 += puntos[i + 1];
    }
    this.cx0 = cx0 / (puntos.length / 2);
    this.cy0 = cy0 / (puntos.length / 2);
  }

  dibujar() {
    
    let cx = this.cx0 + this.desplazX;
    let cy = this.cy0 + this.desplazY;

    // push/pop y transformaciones solo cuando son necesarias
    let needsTransform = this.anguloActual !== 0 || this.escalaActual !== 1.0;
    if (needsTransform) {
      push();
      translate(cx, cy);
    ...
    for (let i = 0; i < this.puntos.length; i += 2) {
      curveVertex(this.puntos[i] + this.desplazX,
                  this.puntos[i + 1] + this.desplazY);
    }
    endShape(CLOSE);

    if (needsTransform) pop();
  }
}
