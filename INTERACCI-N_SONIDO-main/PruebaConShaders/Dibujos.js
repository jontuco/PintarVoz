...
class Curvas_1 {
  ...
  reiniciarBolsa() { // y
    ...
    shuffle(this.bolsa, true); // y
  } // y

  obtenerCurvaSinRepetir() { // y
    if (this.bolsa.length === 0) { // y
      this.reiniciarBolsa(); // y
    } // y

    let indice = this.bolsa.pop(); // y
    return this.curvas[indice]; // y
  } // y

  
  dibujar(pg, opacidad, vib,hug, satu) { 
    //if(ruidoCorto && ){
    //  this.reordenar=!reordenar;
    //}
   
    pg.background(this.fondoColor); 
    for (let c of this.curvas) c.dibujar(pg, opacidad, vib,hug, satu); 
    /*
    if(ruidoCorto){
      pg.background(this.fondoColor); 
    for (let i = 0; i < this.curvas.length; i++) { // y
...
class Curva {
  ...
  dibujar(pg, op, vib,hug, satu) { 
    let col = color(constrain(hue(this.c) - int(random(hug)), 0, 100), constrain(saturation(this.c) - satu, 0, 100), brightness(this.c), op);
