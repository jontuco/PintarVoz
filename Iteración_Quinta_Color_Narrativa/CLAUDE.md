...
## Marcado de líneas modificadas, nuevas y borradas

Usá este sistema de marcas en todo el código que toques:

**Líneas modificadas** — añadí `// x` al final de la línea cambiada:
```js
let velocidad = 3; // x
```

**Líneas nuevas** — añadí `// y` al final de cada línea agregada que no existía antes:
```js
let gravedad = 0.5; // y
```

**Líneas borradas** — no las borres en silencio. Dejá una línea vacía con un comentario que indique cuántas líneas se eliminaron debajo de ese punto:
```js
// y 3 líneas borradas
```

El número debe ser exacto: contá las líneas que efectivamente quitaste.
